...
class CisoApiClient:
    ...
    def resolve_assessment(
        ...
        if not matches:
            raise CisoApiError(
                f"找不到 assessment。name={assessment_name}, framework={assessment_framework}, version={assessment_version}"
            )
        ...
        if len(matches) > 1:
            ...
            raise CisoApiError(
                f"assessment 同名冲突，请补充 --assessment-framework 或 --assessment-id。候选: {choices}"
            )
    ...
    def attach_evidence_to_requirement(self, requirement_assessment_id: str, evidence_ids: List[str]) -> Dict[str, Any]:
        payloads = [
